...
func (c MetadataController) Handler() gin.IRoutes {
	return c.Router.GET("/saml/metadata", func(context *gin.Context) {
		...
		c.Logger.Info(fmt.Sprintf("Handling SAML metadata request [correlation_id=%s, method=%s, path=%s, client_ip=%s]", correlationID, context.Request.Method, context.Request.URL.Path, context.ClientIP()))
		...
		c.Logger.Info(fmt.Sprintf("SAML metadata request processed successfully [correlation_id=%s, root_url=%s]", correlationID, rootUrl))
	})
}